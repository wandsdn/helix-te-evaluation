...
# using the CSPFRecomp TE optimisation methods with a reverse candidate sort.
# Same behaivour as the standard HelixMC tests (run_HelixMC_path_test.sh).
# ------------------------------------------------------------


# Base path to folder that contains all HelixMC test scenario files
BASE_FOLDER="HelixMC_TEST/CSPF"
...
YATES_ATTR="-helixMC -helix-poll-wait 100 -helix-te-opti-wait 1"
YATES_ATTR="$YATES_ATTR -helix-te-opti-method CSPFRecomp"
YATES_ATTR="$YATES_ATTR -helix-te-candidate-sort-rev true"
...
run_test() {
    ...
    echo "Collecting results, log path: $LOG_FOLDER/$1.cspf.out.txt";
    yates "$BASE_FOLDER/$1.dot" "$BASE_FOLDER/$1.te.txt" \
            ...
            -helix-te-thresh $2 -out $1 > "$LOG_FOLDER/$1.cspf.out.txt" 2>&1
